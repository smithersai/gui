...
//!                       backend, seed a test user/token, build SmithersGUIiOS,
...
pub fn build(b: *std.Build) void {
    ...
    // ---- zmux --------------------------------------------------------------
    // zmux now lives at github.com/smithersai/zmux and is consumed by
    // libsmithers via build.zig.zon. The `zmux` and `zmux-test` steps proxy
    // into libsmithers so existing invocations keep working.
    const zmux_build = b.addSystemCommand(&.{ "zig", "build" });
    zmux_build.setCwd(b.path("libsmithers"));
    const zmux_step = b.step("zmux", "Build zmux helper binaries via libsmithers");
    zmux_step.dependOn(&zmux_build.step);

    const zmux_test = b.addSystemCommand(&.{ "zig", "build", "test" });
    zmux_test.setCwd(b.path("libsmithers"));
    const zmux_test_step = b.step("zmux-test", "Run zmux tests via libsmithers");
    ...
    // Required so the SmithersGUIiOS target's `canImport(CSmithersKit)` gates
    ...
    // ---- legacy agent/TUI branch -------------------------------------------
    const legacy_tui_step = b.step("legacy-tui", "Build legacy Go TUI binary");
    const legacy_tui_cmd = b.addSystemCommand(&.{ "go", "build", "-C", "tui", "-o", "../agent-tui", "." });
    legacy_tui_step.dependOn(&legacy_tui_cmd.step);

    const legacy_run_tui_step = b.step("legacy-run-tui", "Build and run legacy TUI against localhost:8000");
    const legacy_run_tui_build = b.addSystemCommand(&.{ "go", "build", "-C", "tui", "-o", "../agent-tui", "." });
    const legacy_run_tui_cmd = b.addSystemCommand(&.{ "sh", "-c", "exec ./agent-tui --embedded=false" });
    legacy_run_tui_cmd.step.dependOn(&legacy_run_tui_build.step);
    legacy_run_tui_step.dependOn(&legacy_run_tui_cmd.step);

    const legacy_run_agent_step = b.step("legacy-run-agent", "Run the legacy Python agent server");
    const legacy_run_agent_cmd = b.addSystemCommand(&.{ "uv", "run", "python", "main.py" });
    legacy_run_agent_step.dependOn(&legacy_run_agent_cmd.step);

    const legacy_test_step = b.step("legacy-test", "Run legacy Python and Go tests");
    const legacy_python_test = b.addSystemCommand(&.{ "uv", "run", "pytest" });
    const legacy_go_sdk_test = b.addSystemCommand(&.{ "go", "test", "-C", "sdk/agent", "./..." });
    const legacy_go_tui_test = b.addSystemCommand(&.{ "go", "test", "-C", "tui", "./..." });
    legacy_test_step.dependOn(&legacy_python_test.step);
    legacy_test_step.dependOn(&legacy_go_sdk_test.step);
    legacy_test_step.dependOn(&legacy_go_tui_test.step);

    const legacy_deps_step = b.step("legacy-deps", "Install/update legacy agent and TUI dependencies");
    const legacy_python_deps = b.addSystemCommand(&.{ "uv", "pip", "install", "-e", "." });
    const legacy_go_deps = b.addSystemCommand(&.{ "go", "mod", "-C", "tui", "tidy" });
    legacy_deps_step.dependOn(&legacy_python_deps.step);
    legacy_deps_step.dependOn(&legacy_go_deps.step);

    const legacy_fmt_step = b.step("legacy-fmt", "Format legacy Go TUI code");
    const legacy_fmt_cmd = b.addSystemCommand(&.{ "go", "fmt", "-C", "tui", "./..." });
    legacy_fmt_step.dependOn(&legacy_fmt_cmd.step);
    ...
    // seed token → xcodebuild SmithersGUIiOS → boot simulator → launch app with
...
fn everythingUp(step: *std.Build.Step, _: std.Build.Step.MakeOptions) anyerror!void {
    ...
    // 4. xcodebuild SmithersGUIiOS for simulator. The iOS target injects the
    ...
    const app_path = "build/DerivedData-everything-up/Build/Products/Debug-iphonesimulator/SmithersGUIiOS.app";
    log("==> installing app on simulator", .{});
    try runStream(step, &.{ "xcrun", "simctl", "install", "booted", app_path }, null, null);
    try runAllowFail(step, &.{ "xcrun", "simctl", "terminate", "booted", "com.smithers-app.ios" });

    log("==> launching SmithersGUIiOS with e2e bypass", .{});
    ...
    try runStream(
        step,
        &.{ "xcrun", "simctl", "launch", "booted", "com.smithers-app.ios" },
        null,
        &launch_env,
    );
