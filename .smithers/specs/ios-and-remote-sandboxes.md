...
- **Plain HTTP + JSON** for writes. Some writes use existing plue routes (create session, append message, cancel run — all live today); others are net-new routes landing in their own tickets (`decide approval` in 0110; canonical run paths in 0111). "Dispatch run" is currently implicit via posting a `user`-role message (`agent_sessions.go:280`) and ticket 0108 decides whether to keep that or add an explicit route. Results arrive back at the client via the shapes they're already subscribed to.
