...
Authoritative JSON schemas live **in plue** alongside the handlers (likely `plue/pkg/wireschema/` or similar). Generated/mirrored type bindings are consumed by Zig (`libsmithers-core`) and any TypeScript clients. The smithers-app repo does not own the schema.
...
- **Authentication primitives.** Bearer-token verification (`Authorization: Bearer jjhub_xxx` via `middleware/auth.go:56`), Auth0 + WorkOS browser flows (`cmd/server/main.go:901`, `internal/routes/auth.go:25`) that today terminate in a **session cookie** for browser users or a **one-off CLI token** for `callback_port` flows (`internal/routes/auth.go:245, 416`), session cookies, and a separate **OAuth2 application + PKCE flow** (`cmd/server/main.go:917, 1244`, `internal/routes/oauth2.go:165`) that does mint refreshable bearer+refresh tokens. **Gap for this spec:** no public OAuth2 client is registered for the smithers-app/iOS apps today, and the Auth0/WorkOS browser flow alone doesn't hand the mobile app a refreshable token pair. Closing that gap is a named prerequisite (ticket 0106).
...
- **Registration is a prerequisite.** No public OAuth2 client for the smithers-app/iOS apps exists in plue today. Registering that client, wiring PKCE handoff, and implementing the iOS/macOS sign-in shell is ticket 0106 — Stage 0.
...
- **Sign-out scope is app-wide for v1:** calling sign-out revokes every OAuth2 session for this user on this plue OAuth2 app, not just the local device. The client targets `POST /api/oauth2/revoke-all` with the current bearer token and falls back to revoking both the current access token and refresh token only when that route is unavailable. Threat-model rationale, "lost device" recovery, and the full set of secure-store decisions live in [`ios-and-remote-sandboxes-secure-store.md`](ios-and-remote-sandboxes-secure-store.md) (ticket 0133).
...
- Migration strategy (smithers-app-tree only): `ios-and-remote-sandboxes-migration.md`.
