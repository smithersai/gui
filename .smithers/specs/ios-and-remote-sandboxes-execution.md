...
Every item below is a task. Each becomes a ticket in `.smithers/tickets/` (smithers-app-side) or `plue/.smithers/tickets/` (plue-side). Tickets get implemented via the `ticket-implement` smithers workflow. Nothing implementation-heavy starts until its task's PoC is green.
...
- *Status:* landed. The smithers-app-tree-only migration plan is at `.smithers/specs/ios-and-remote-sandboxes-migration.md`; it carries a file-by-file inventory of `libsmithers/src/`, a 12-step commit sequence, per-stage rollback, desktop-app compatibility gates, and a prerequisites appendix for cross-repo blockers.
