...
This file is the single source of truth for turning the `SmithersGUIiOS`
...
1. Register the bundle id in App Store Connect: `com.smithers-app.ios`.
...
- The bundle id is pinned in `project.yml` to **`com.smithers-app.ios`**.
...
- Steps: App Store Connect → Apps → `+` → New App →
  iOS → bundle id = `com.smithers-app.ios`, SKU = `smithers-app-ios`.

### 3. Create a TestFlight group

- App Store Connect → your app → TestFlight → Internal Testing →
  `+` → create a group (for example "SmithersGUI internal").
...
1. <https://developer.apple.com/account/resources/profiles/list>
   ...
   `com.smithers-app.ios` → pick the Apple Distribution certificate from
   step 5 → name it (e.g. "SmithersGUI iOS App Store").
2. Download the `.mobileprovision` file.
3. Base64-encode it: `base64 -i SmithersGUI_iOS_App_Store.mobileprovision | pbcopy`.
4. Put the encoded blob in `IOS_PROVISIONING_PROFILE_BASE64` and the
   ...
   `SMITHERS_APP_IOS_PROVISIONING_PROFILE_NAME`.
...
| `SMITHERS_APP_IOS_PROVISIONING_PROFILE_NAME` | the human-readable name you typed when creating the profile | `SmithersGUI iOS App Store` |
...
3. `SMITHERS_APP_IOS_PROVISIONING_PROFILE_NAME`.
...
`project.yml` carries two values for the `SmithersGUIiOS` target:
...
1. Edit `project.yml` → `targets.SmithersGUIiOS.settings.base.MARKETING_VERSION`.
...
export PROVISIONING_PROFILE_SPECIFIER="SmithersGUI iOS App Store"
...
env -u SDKROOT -u LIBRARY_PATH -u RUSTFLAGS \
    xcodebuild \
        ...
        -archivePath build/ios-archive/SmithersGUIiOS.xcarchive \
        MARKETING_VERSION="0.1.0" \
        CURRENT_PROJECT_VERSION="1" \
        DEVELOPMENT_TEAM="ABCDE12345" \
        PROVISIONING_PROFILE_SPECIFIER="SmithersGUI iOS App Store" \
...
```
error: "SmithersGUIiOS" requires a provisioning profile. Select a
...
1. **"no matching provisioning profile found"** — the profile name
   ...
   update `SMITHERS_APP_IOS_PROVISIONING_PROFILE_NAME` to the exact name
...
- App profile must exist at `~/Library/MobileDevice/Provisioning Profiles/smithers-app-ios-appstore.mobileprovision` and match `com.smithers-app.ios`.
- Share-extension profile must exist at `~/Library/MobileDevice/Provisioning Profiles/smithers-app-ios-shareext-appstore.mobileprovision` and match `com.smithers-app.ios.ShareExtension`.
...
export PROVISIONING_PROFILE_SPECIFIER="SmithersGUI iOS App Store"
export SHARE_EXTENSION_PROVISIONING_PROFILE_SPECIFIER="SmithersGUI iOS Share   Extension App Store"
