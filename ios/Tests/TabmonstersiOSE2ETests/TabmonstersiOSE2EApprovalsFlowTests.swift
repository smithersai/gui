...
final class TabmonstersiOSE2EApprovalsFlowTests: XCTestCase {
    ...
    private static let repoRoot = URL(fileURLWithPath: #filePath)
        ...
        .deletingLastPathComponent()
        .deletingLastPathComponent()
    private static let seedScriptPath = repoRoot.appendingPathComponent("ios/scripts/seed-e2e-data.sh").path
    ...
    private func runHostShell(
        ...
        let wrappedCommand = """
        cd \(shellQuote(Self.repoRoot.path)) && (\(command)) > \(shellQuote(stdoutURL.path)) 2> \(shellQuote(stderrURL.path))
        """
