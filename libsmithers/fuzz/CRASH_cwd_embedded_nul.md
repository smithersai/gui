# CWD resolver embedded NUL crash

- Target: `cwd`
- Found by: `zig build` corpus smoke test
- UTC timestamp: `20260421T054917Z`
- Repro input: `crashes/cwd/20260421T054917Z/input.bin`
- Input bytes: `70 72 65 66 69 78 00 73 75 66 66 69 78 ff 0a`

## Summary

The direct internal resolver path `workspace/cwd.zig:resolve` crashes when the
requested path contains an embedded NUL byte. The public C ABI
`smithers_cwd_resolve` truncates at the first NUL because it receives
`[*:0]const u8`, but the internal resolver accepts `[]const u8` and passes the
normalized path to `std.fs.openDirAbsolute`. Zig 0.15.2 asserts that POSIX path
inputs do not contain NUL bytes.

No `libsmithers/src` files were modified.
